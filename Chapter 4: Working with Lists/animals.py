# Try It Yourself 4-2: Animals
# Store three animals that have common characteristics in a list,
# then use a for loop to print out their names.
# Modify the program to print a statement about each animal.
# Add a line at the end stating what they have in common.

animals = ['dog', 'shark', 'scorpion', 'parrot']

for animal in animals:
    print(f"A {animal} is a type of animal.")

print("All of these animals have tails!")
